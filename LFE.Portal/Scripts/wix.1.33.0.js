...
Wix.WixDataCursor = (function () {
	var _callService = function (onSuccess, onFailure, cursorId) {
		...
		this.dispatcher.sendMessage(this._serviceMessageType, args, onComplete);
	};

	function WixDataCursor(dispatcher, serviceMessageType, data, total, pageSize) {
...
Wix.Dashboard = (function () {
	...
	return {
		 ...
		 * @param onSuceess (Function) callback where the data schema is:
		 ...
		 */
		openMediaDialog: function (mediaType, multipleSelection, onSuccess, onCancel) {
			Wix.Settings.openMediaDialog(mediaType, multipleSelection, onSuccess, onCancel);
		},
		 ...
		 *
		 * Closes the apps modal/popup window.
		 ...
		 *
		 * Get the editor url of the site as parameter in the callback
		 *
		 * @param callback (Function) a callback which gets editor url as parameter
		 *
		 * @since SDK 1.33.0
		 */
		getEditorUrl: function (callback) {
			if (!callback) {
				_w.reportSdkError('Mandatory arguments - a callback must be specified');
			}
			_dispatcher.sendMessage(_w.MessageTypes.GET_EDITOR_URL, undefined, callback);
		}
	}
})();
var Wix = Wix || {};
...
Wix.Contacts = (function () {
	...
	return {
		 ...
		 *
		 * @since  SDK 1.31.0
		 * @param options (Object) object that supports two parameters: 'label' and 'pageSize'.
		 ...
		 * @return (WixDataCursor) cursor
		 */
		getContacts: function (options, onSuccess, onFailure) {
			_w.trackSDKCall("Contacts.getContacts");

			if (options === null || typeof options !== 'object') {
				_w.reportSdkError('Missing mandatory argument - options, must be an object');
				return;
			}
			...
			var onComplete = function onComplete(response) {
				_handlers.handleCursorResponse(response, onSuccess, onFailure, _w.MessageTypes.GET_CONTACTS, options);
			};

			_dispatcher.sendMessage(_w.MessageTypes.GET_CONTACTS, args, onComplete);
		},
		 ...
		 *
		 * @since SDK 1.32.0
		 * @param onSuccess (Function) A function that receives an array of labels
		 * @param onFailure (Function) A function called when an error occurs that receives a Wix.Error
		 * @return (Object) An array of labels
		 */
		getContactLabels: function (onSuccess, onFailure) {
			_w.trackSDKCall("Contacts.getContactLabels");
			...
			_dispatcher.sendMessage(_w.MessageTypes.GET_CONTACT_LABELS, {}, onComplete);
		}
	}
})();
var Wix = Wix || {};
...
Wix.ResponseHandlers = (function () {
	...
	return {
		...
		handleCursorResponse: function (response, onSuccess, onFailure, messageType, options) {
			if (!response.error) {
				var cursor = new Wix.WixDataCursor(_dispatcher, messageType, response.data.results, response.data.total, response.data.pageSize);
...
Wix.BaseAPI = (function () {
	...
	return {
		 ...
		 * @param height (Number) new component height
		 */
		setHeight: function (height,compId) {
		    ...
		    if (compId != null) _w.compId = compId;

			_w.sendMessageInternal2(_w.MessageTypes.HEIGHT_CHANGED, { 'height': height ,'compId':compId});
		},
		...
		*/
		getCurrentPageId: function (callback) {
			_dispatcher.sendMessage(_w.MessageTypes.GET_CURRENT_PAGE_ID, null, callback);
		},
		...
		scrollTo: function (x, y) {
			...
			_dispatcher.sendMessage(_w.MessageTypes.SCROLL_TO, args);
		},
...
Wix.Settings = (function () {
	...
	return {
		 ...
		 *          width - media type width (for images)
		 *          height - media type height (for images)
		 * @param onCancel (Function) callback function called when user cancels
		 */
		openMediaDialog: function (mediaType, multipleSelection, onSuccess, onCancel) {
			if (!onSuccess) {
				return;
			...
			_w.sendMessageInternal2(_w.MessageTypes.OPEN_MEDIA_DIALOG,
				{
					mediaType: mediaType,
		...
		/**
		 * Gets the URL for your BackOffice app in the site's Dashboard.
		 * The URL is fully qualified and starts with "//" for using HTTPS if supported.
		 *
		 * @param callback (Function) a callback function that returns the app's dashboard URL
		 *   callback signature: function(url) {}
		 *
		 * @since SDK 1.32.0
		 */
		getDashboardAppUrl: function (callback) {
			if (!callback) {
				_w.reportSdkError('Mandatory arguments - a callback must be specified');
			}

			_w.sendMessageInternal2(_w.MessageTypes.GET_DASHBOARD_APP_URL, undefined, callback);
		}
	};
})();
var Wix = Wix || {};
...
Wix.Utils = (function () {
	...
	return {
		 ...
		 *
		 * @since SDK 1.33.0
		 * @param sectionIdenfitier (Object, optional) app page id defined in dev.wix.com { sectionId : 'sectionId' }
		 * @param state (String, optional) new app's state to push into the editor history stack
		 * @return onFailure (Function) this will be called if the hosting site does not include the section app, or if the caller's application does not include a section
		 */
		navigateToSection: function (sectionIdentifier, state, onFailure) {
			_w.trackSDKCall("Utils.navigateToSectionPage");
			var args;
			if (isFunction(sectionIdentifier)) {
				onFailure = sectionIdentifier;
			}
			else if (isString(sectionIdentifier)) {
				args = {
					state: sectionIdentifier
				};
				onFailure = state;
			} else if (isObject(sectionIdentifier) && isFunction(state)) {
				args = {
			...
			_dispatcher.sendMessage(_w.MessageTypes.NAVIGATE_TO_SECTION_PAGE, args, onFailure);
		},
...
Wix.Worker = (function () {
	...
	return {
		 ...
		 */
		addEventListener: function (eventName, callBack) {
			return _wix.addEventListener(eventName, callBack);
		},
		 ...
		 */
		removeEventListener: function (eventName, callBackOrId) {
			return _wix.removeEventListener(eventName, callBackOrId);
		},
		...
		PubSub: {
			 ...
			 */
			publish: function (eventKey, data, isPersistent) {
				return _wix.PubSub.publish(eventKey, data, isPersistent);
			},
			 ...
			 */
			subscribe: function (eventKey, callBack, receivePastEvents) {
				return _wix.PubSub.subscribe(eventKey, callBack, receivePastEvents);
			},
			 ...
			 */
			unsubscribe: function (eventKey, callBackOrId) {
				return _wix.PubSub.unsubscribe(eventKey, callBackOrId);
			}
		},
		...
		Utils: {
			 ...
			 */
			getDeviceType: function () {
				return _wix.Utils.getDeviceType();
			},
			 ...
			 */
			getInstanceId: function () {
				return _wix.Utils.getInstanceId();
			},
			 ...
			 */
			getIpAndPort: function () {
				return _wix.Utils.getIpAndPort();
			}
		}
	}
})();
...
(function (container) {
	var _w = {
		...
		init: function (opts) {
			...
			_w.addEventListenerInternal('EDIT_MODE_CHANGE', function (params) {
		...
		sendMessageInternal2: function (msgType, params, callback) {
		    if (!msgType) {
		        console.log('send internal message::msgType undefined');
				return;
			}
			...
			console.log('send internal message::params is ' + JSON.stringify(params));
			...
			if (target && typeof target != "undefined") {
				...
				window.console && window.console.log && window.console.log(JSON.stringify(blob));
	...
	Wix.__deployOnlyWorker = function () {
		var wixEvents = Wix.Events;
		var wixError = Wix.Error;
		var worker = Wix.Worker;
		var deployWorker = Wix.__deployOnlyWorker;
		Wix = {};
		Wix.Worker = worker;
		Wix.Events = wixEvents;
		Wix.Error = wixError;
		Wix.__deployOnlyWorker = deployWorker;
	...
	container.Wix.Dashboard.init(_w, Wix.PostMessageDispatcher);
	...
	container.Wix.Utils.init(_w, Wix.PostMessageDispatcher);
	...
	container.Wix.Worker.init(Wix);
	Wix.Worker = container.Wix.Worker;

	var endpointType = _w.getQueryParameter('endpointType');

	// if it's an App Worker, block all namespaces except:
	// Wix.Worker
	// Wix.Events
	// Wix.Error
	if (endpointType === 'worker') {
		Wix.__deployOnlyWorker();
	} else {
		//Deploy API on the container (iframe window)
		container.Wix = Wix;
	}
