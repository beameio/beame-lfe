...
namespace LFE.Portal.Areas.UserPortal.Controllers
{
    public class DiscussionController : BaseController
    {
        ...
        //{
        //    var messages = _emailServices.GetMessageNotifications(messageId);
        //    var cntroller = this;
        //    foreach (var token in messages.ToList())
        //    {
        //        var html = _emailServices.NotificationDto2Html(token); // RenderPartialToString(cntroller ,"_NotificationMailTemplate",_emailServices.NotificationDto2EmailBodyToken(token));

        //        if (String.IsNullOrEmpty(html)) continue;

        //        long emailId;
                
        //        _emailServices.SaveEmailNotificationRecord(token, html, out emailId);

        //        if(!Boolean.Parse(Utils.GetKeyValue("autoEmail"))) return;

        //        var emailSaved = _emailServices.SaveEmailNotificationRecord(token, html, out emailId);

        //        if (!emailSaved || emailId < 0) continue;
        //        Logger.Debug("send email" );
        //        _emailServices.SendEmail(emailId);
        //    }
        //}

        //public ActionResult _NotificationMailTemplate(EmailBodyToken token)
        //{
        //    return PartialView("_NotificationMailTemplate", token);
        //}
        #endregion
