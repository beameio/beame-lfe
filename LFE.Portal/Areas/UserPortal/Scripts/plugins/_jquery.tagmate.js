    ...
    $.fn.extend({
        ...
        tagmate: function(options) {
            ...
            function update_menu(menu, options) {
                ...
                menu.html(null);
                //menu.addClass('nano');
                var container = $('<div />').addClass('content').attr({id:'d-menu-container'});
                var ul = $('<ul />').addClass('ul-user-au');
                ...
                for (var i = 0; i < options.length; i++) {
                    ...
                    ul.append("<li data-val='" + value + "' class='" + classes + "'>" + content + "</li>");
                }
                ...
                setTimeout(function () {
                    ...
                    //menu.nanoScroller({ sliderMinHeight: 80, alwaysVisible: true });
                }, 100);
            ...
            function scroll_menu(menu, direction) {
                ...
                var ul = $('.ul-user-au');
                var active = ul.children("." + defaults.menu_option_active_class);

                if (active.length == 0) {
                    active = ul.children(child_selector);
                ...
                var i, options = ul.children();
                var n =  Math.floor($(ul).height() / $(options[0]).height()) - 1;
                if ($(ul).height() % $(options[0]).height() > 0)
                ...
                if (i > n && (i - n) >= 0 && (i - n) < options.length) {
                    menu.mCustomScrollbar("scrollTo", $(options[i - n]));
                }
                    // ul.scrollTo(options[i - n]);
